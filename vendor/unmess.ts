// Blog: https://quadst.rip/poly-isect.html
// Code: https://web.archive.org/web/20250706072040/https://unmess-poly.glitch.me/unmess.ts
...
function poly_is_hole(
	hole: Pt[],
	poly: Pt[],
	verify_winding: boolean = true,
	eps: number = 0.0001
) {
...
export function unmess(poly: Pt[], args: Record<any, any> = {}) {
	...
	function trace_outline(
		i0: number,
		j0: number,
		dir: number,
		is_outline: boolean,
		force_no_backturn: boolean = false
	): Trace_ret {
		let local_used = {};
		...
		function trace_from(i0: number, j0: number, dir: number, prev: [number, number]): boolean {
			...
			if (j0 == -1) {
				...
				if (
					args.hole_policy == HOLE_AGGRESSIVE &&
					!is_outline &&
					(used_isects[id] || local_used_isects[id])
				) {
					return false;
				}
				...
				if (
					(args.hole_policy == HOLE_AGGRESSIVE || force_no_backturn) &&
					!is_outline &&
					params[2] != dir
				) {
					return false;
