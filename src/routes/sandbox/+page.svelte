<script lang="ts">
	...
	import { AxesHelper, MeshBasicMaterial, MeshNormalMaterial } from 'three';
	...
	const legends = fonts.germania_one;

	//const builder1 = new Builder(CrystalD12, legends);
	const builder2 = new Builder(dice.caltrop_d4, legends);
	...
	const m2 = new MeshBasicMaterial({ color: 0x000000, wireframe: showWireframe });
...
<div class="canvas" bind:this={target}></div>
<p><button onclick={downloadSTL}>download stl</button></p>
<p><button onclick={toggleGrid}>toggle grid</button></p>
<p><button onclick={toggleMain}>toggle main</button></p>
<p><button onclick={toggleBad}>toggle bad</button></p>
