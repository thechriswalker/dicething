...
type ExplodableFace = { isNumberFace: boolean; shape: Shape; explodeTransform?: Transform };
...
export function stackedExplode(faces: Array<ExplodableFace>, opts: StackedExplodeOptions = {}) {
	const gap = opts.gap ?? 2;
	const numbers = faces.filter((f) => f.isNumberFace);
	const caps = faces.filter((f) => !f.isNumberFace);

	// uniform cell pitch derived from every face.
