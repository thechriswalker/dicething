...
const builtins = {
	...
	averia: { name: 'Averia', tags: ['std'], load: deferredFontLoader('averia') } as Builtin,
	averia_100: {
		name: 'Averia (100)',
		tags: ['0-99'],
		load: deferredFontLoader('averia_100')
	} as Builtin,
	germania_one: {
		name: 'Germania One',
		tags: ['std'],
		load: deferredFontLoader('germania_one')
	} as Builtin,
	germania_one_100: {
	...
	tektur: { name: 'Tektur', tags: ['std'], load: deferredFontLoader('tektur') } as Builtin,
	tektur_100: {
		name: 'Tektur (100)',
		tags: ['0-99'],
		load: deferredFontLoader('tektur_100')
	} as Builtin,
	voltaire: { name: 'Voltaire', tags: ['std'], load: deferredFontLoader('voltaire') } as Builtin,
	voltaire_100: {
		name: 'Voltaire (100)',
		tags: ['0-99'],
		load: deferredFontLoader('voltaire_100')
	} as Builtin
} as const;
